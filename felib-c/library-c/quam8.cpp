/* quam8.f -- translated by f2c (version of 28 August 1991  0:07:02).
...
/* Subroutine */ int quam8_(fun, ifun, der, ider, jder, xi, eta, itest)
doublereal *fun;
integer *ifun;
doublereal *der;
integer *ider, *jder;
...
/* ARGUMENTS in */
/*      IFUN    length of vector FUN (.GE. 8) */
...
/*     Main body */
    ...
    der[der_dim1 + 1] = etam * (*xi * 2. + *eta);
    der[(der_dim1 << 1) + 1] = etam * -8. * etap;
    der[der_dim1 * 3 + 1] = etap * (*xi * 2. - *eta);
    der[(der_dim1 << 2) + 1] = etap * -4. * *xi;
    der[der_dim1 * 5 + 1] = etap * (*xi * 2. + *eta);
    der[der_dim1 * 6 + 1] = etap * 8. * etam;
    der[der_dim1 * 7 + 1] = etam * (*xi * 2. - *eta);
    der[(der_dim1 << 3) + 1] = etam * -4. * *xi;
    der[der_dim1 + 2] = xim * (*xi + *eta * 2.);
    der[(der_dim1 << 1) + 2] = xim * -4. * *eta;
    der[der_dim1 * 3 + 2] = xim * (*eta * 2. - *xi);
    der[(der_dim1 << 2) + 2] = xim * 8. * xip;
    der[der_dim1 * 5 + 2] = xip * (*xi + *eta * 2.);
    der[der_dim1 * 6 + 2] = xip * -4. * *eta;
    der[der_dim1 * 7 + 2] = xip * (*eta * 2. - *xi);
    der[(der_dim1 << 3) + 2] = xim * -8. * xip;

    fun[1] = etam * 4. * xim * (-(*xi) - *eta - 1.);
    fun[2] = xim * 32. * etam * etap;
    fun[3] = etap * 4. * xim * (-(*xi) + *eta - 1.);
    fun[4] = xim * 32. * xip * etap;
    fun[5] = xip * 4. * etap * (*xi + *eta - 1.);
    fun[6] = xip * 32. * etap * etam;
    fun[7] = xip * 4. * etam * (*xi - *eta - 1.);
    fun[8] = xim * 32. * xip * etam;
