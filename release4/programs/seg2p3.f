...
C
      ...
      WRITE (NOUT,FMT=9940) RESNOD
      K = DOFNOD + 1
      DO 1020 I = 1,RESNOD
          READ (NIN,FMT=9990) (RESTR(I,J),J=1,K)
          WRITE (NOUT,FMT=9940) (RESTR(I,J),J=1,K)
 1020 CONTINUE
...
C
      WRITE (NOUT,FMT=9880)
      READ (NIN,FMT=9980) NMODES,DAMPR
      WRITE (NOUT,FMT=9930) NMODES,DAMPR
C
C     Input TIME step length and number of steps
C
      WRITE (NOUT,FMT=9870)
      READ (NIN,FMT=9960) DTIM,NSTEPS
      WRITE (NOUT,FMT=9860) DTIM,NSTEPS
C
C     Input loading data
C
      WRITE (NOUT,FMT=9850)
      READ (NIN,FMT=9990) NUMLOD
      WRITE (NOUT,FMT=9940) NUMLOD
      TOTLOD = 0
      DO 1040 I = 1,NUMLOD
          READ (NIN,FMT=9990) NODNUM, (NWORK(J),J=1,DOFNOD)
          WRITE (NOUT,FMT=9940) NODNUM, (NWORK(J),J=1,DOFNOD)
          DO 1030 J = 1,DOFNOD
...
C
      WRITE (NOUT,FMT=9840)
      READ (NIN,FMT=9970) AMP,OMEGA
      WRITE (NOUT,FMT=9900) AMP,OMEGA
C
C     *                           *******************************
C     *                           *                             *
...
C
     ...
     *           TOTDOF,TOL,ITEST)
      ...
      DO 1170 ISTEP = 1,NSTEPS
          TIME = TIME + DTIM
          WRITE (NOUT,FMT=9820) ISTEP,TIME
          DO 1140 J = 1,NMODES
              DO 1120 I = 1,TOTDOF
