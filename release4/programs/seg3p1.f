...
C
      ...
      real*8 al(100,100)
      integer ropiv(100)
      integer ia,ja,iropiv
      data ial/100/,jal/100/,iropiv/100/
C
...
C
      WRITE (NOUT,FMT=9890)
      READ (NIN,FMT=9970) STRGTH
      WRITE (NOUT,FMT=9900) STRGTH
C
...
C      CALL MATNUL(SYSK,ISYSK,JSYSK,TOTDOF,HBAND,ITEST)
...
C
     ...
     *                ISTEER,ITEST)
C          CALL ASSYM(SYSK,ISYSK,JSYSK,ELK,IELK,JELK,STEER,ISTEER,HBAND,
C     *               DOFEL,ITEST)
          CALL ASUSM(SYSK,ISYSK,JSYSK,ELK,IELK,JELK,STEER,ISTEER,HBAND,
     *               DOFEL,ITEST)
...
C
      ...
      write(20,*) hband
      do 5050 i=1,totdof
      write(20,*)(sysk(i,j),j=1,2*hband-1)
 5050 continue
      write(20,*)(rhs(i),i=1,totdof)
      call flush(20)

      CALL GAUSOL(SYSK,ISYSK,JSYSK,AL,IAL,JAL,TOTDOF,HBAND,ROPIV,IROPIV,
     *          RHS,IRHS,ITEST)
C      CALL CHOSOL(SYSK,ISYSK,JSYSK,RHS,IRHS,TOTDOF,HBAND,ITEST)
      WRITE (NOUT,FMT=9870)
