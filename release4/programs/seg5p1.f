...
C
      DO 1020 I = 1,BNDNOD
          READ (NIN,FMT=9980) BNODE(I),BVAL(I)
          WRITE (NOUT,FMT=9940) BNODE(I),BVAL(I)
 1020 CONTINUE
C
...
C
      IF (NJV.NE.0) THEN
          DO 1030 I = 1,NJV
              READ (NIN,FMT=9970) ELNUM,JVAL(ELNUM)
              WRITE (NOUT,FMT=9900) ELNUM,JVAL(ELNUM)
 1030     CONTINUE
      END IF
C
C     Input iteration control parameters
C
      READ (NIN,FMT=9970) MAXIT,CFACTR
      WRITE (NOUT,FMT=9890) MAXIT,CFACTR
C
