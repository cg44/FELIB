...
C
      ...
      WRITE (NOUT,FMT=9930) TOTNOD,DIMEN
      DO 1010 I = 1,TOTNOD
          READ (NIN,FMT=9980) NODNUM, (COORD(NODNUM,J),J=1,DIMEN)
          WRITE (NOUT,FMT=9920) NODNUM, (COORD(NODNUM,J),J=1,DIMEN)
C
C     Normalise COORD
C
          DO 1000 J = 1,DIMEN
...
C
      ...
      WRITE (NOUT,FMT=9930) TOTELS
      DO 1020 I = 1,TOTELS
          READ (NIN,FMT=9990) ELNUM,ELTOP(ELNUM,1),NODEL,
     *      (ELTOP(ELNUM,J+2),J=1,NODEL)
          WRITE (NOUT,FMT=9930) ELNUM,ELTOP(ELNUM,1),NODEL,
     *      (ELTOP(ELNUM,J+2),J=1,NODEL)
          ELTOP(ELNUM,2) = NODEL
 1020 CONTINUE
C
...
C
      WRITE (NOUT,FMT=9900)
      READ (NIN,FMT=9970) XSTRT,XSTOP
      WRITE (NOUT,FMT=9890) XSTRT,XSTOP
      READ (NIN,FMT=9970) DSTRT,DSTOP
      WRITE (NOUT,FMT=9890) DSTRT,DSTOP
C
C
...
C
      ...
      WRITE (NOUT,FMT=9930) BNDNOD
      VMAX = 0.D0
      VMIN = 0.D0
      DO 1030 I = 1,BNDNOD
          READ (NIN,FMT=9960) RESTR(I,1),BVAL(I)
          WRITE (NOUT,FMT=9920) RESTR(I,1),BVAL(I)
          RESTR(I,2) = 1
...
C
      WRITE (NOUT,FMT=9870)
      READ (NIN,FMT=9950) MAXIT,ERROR
      WRITE (NOUT,FMT=9920) MAXIT,ERROR
C
