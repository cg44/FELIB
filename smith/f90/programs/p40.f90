program p40
!------------------------------------------------------------------------------
! program 4.0 equilibrium of axially loaded 1-d rods
!------------------------------------------------------------------------------
use new_library  ;   use  geometry_lib ;   implicit none    
integer::nels,neq,nn,nband,nr,nod=2,nodof=1,ndof=2,iel,i,k,ndim=1,        &
         loaded_nodes,fixed_nodes,np_types   
!---------------------------dynamic arrays-------------------------------------
real,allocatable::km(:,:),eld(:),kv(:),loads(:),coord(:,:),               &
                    action(:),g_coord(:,:),value(:),ea(:),ell(:)
integer,allocatable::nf(:,:),g(:),num(:),g_num(:,:),no(:),g_g(:,:),       &
                     node(:),etype(:)
!------------------------input and initialisation-----------------------------
open (10 , file = 'p40.dat' , status = 'old' ,    action ='read')
open (11 , file = 'p40.res' , status = 'replace', action='write')
...
allocate (nf(nodof,nn),km(ndof,ndof),coord(nod,ndim),g_coord(ndim,nn),    &
          ...
          ea(np_types),ell(nels),g_g(ndof,nels),etype(nels)) 
read(10,*)ea; etype=1; if(np_types>1)read(10,*)etype ; read(10,*)ell,nr
...
elements_2: do iel=1,nels
              call rod_km(km,ea(etype(iel)),ell(iel)); g=g_g(:,iel)
...
elements_3: do iel=1,nels
              call rod_km(km,ea(etype(iel)),ell(iel))
              g=g_g(:,iel); eld=loads( g ); action=matmul(km,eld)   
              write(11,'(i5,a,2e12.4)')iel,"   ",action      
end do elements_3                 
end program p40
