...
C
     ...
     *     PD, QUOT, RHS, SCALE, SCVEC, SRC, STRGTH, SYSK,
     *     WGHT, X, XI, Y
      LOGICAL FIRST
...
C
 ...
 1070 CONTINUE
      SCVEC(I) = FUN(I)*SRC(X,Y,STRGTH)*QUOT
 1080 CONTINUE
...
C
 ...
 9090 FORMAT (//27H **** NODAL POTENTIALS ****//1H )
      END
C
      DOUBLE PRECISION FUNCTION SRC(X, Y, STRGTH)
      DOUBLE PRECISION STRGTH, X, Y
      SRC = 0.0D0
      IF ((X.GT.1.0D0) .AND. (X.LT.2.0D0) .AND. (Y.GT.1.0D0) .AND.
     *     (Y.LT.2.0D0)) SRC = STRGTH
      RETURN
      END
C***********************************************************************
