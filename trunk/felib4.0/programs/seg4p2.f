C***********************************************************************
C     $Id: seg4p2.f,v 1.2 2009/02/27 11:44:50 cg44 Exp $
C
...
C
      ...
      WRITE (NOUT,9910) (P(I,I),I=1,DIMEN)
C
C     Input of convection velocities
C
      WRITE (NOUT,9900)
      READ (NIN,9970) (VEL(I),I=1,DIMEN)
      WRITE (NOUT,9910) (VEL(I),I=1,DIMEN)
C
C     Input of boundary conditions (BTYPE = 1 (neumann) or 2
C     (cauchy))
C
      WRITE (NOUT,9890)
      READ (NIN,9970) CONST1,CONST2
      WRITE (NOUT,9910) CONST1,CONST2
C
      READ (NIN,9990) TOTBND
      WRITE (NOUT,9950) TOTBND
C
      IF (TOTBND.NE.0) THEN
C
          DO 1020 I = 1,TOTBND
     ...
     *          (BDCND(I,J+3),J=1,NUMNOD)
              WRITE (NOUT,9950) BTYPE,NUMNOD,NODSID,
     *          (BDCND(I,J+3),J=1,NUMNOD)
              BDCND(I,1) = BTYPE
              BDCND(I,2) = NUMNOD
              BDCND(I,3) = NODSID
...
C
      ...
      READ (NIN,9990) INTNOD
      WRITE (NOUT,9950) INTNOD
C
      IF (INTNOD.NE.0) THEN
          DO 1050 I = 1,INTNOD
              READ (NIN,9980) NODNUM,VALUE
              WRITE (NOUT,9870) NODNUM,VALUE
              J = NF(NODNUM,1)
              RHS(J) = VALUE
 1050     CONTINUE
      END IF
...
C
      ...
      WRITE (NOUT,9870) NSTEPS,DTIM,THETA
C
