C $Id: seg5p2.f,v 1.4 2009/03/03 17:46:12 cg44 Exp $
      PROGRAM SEG5P2
C***********************************************************************
C
...
C
     ...
     *     DELTA, DET, DMAX, DMIN, DOPE, DOPEPS, DOPING, DSTOP,
...
C
     ...
     *     2))
      DOPEPS = DABS(2.0D+00*VEPS(VMIN)*(DSTRT-DSTOP))
      DO 1055 I=1,TOTNOD
      X=COORD(I,1)
      DOPE=DOPING(X,XSTRT,XSTOP,DSTRT,DSTOP,DEBYL,NSUBI)
      IF (DABS(DOPE) .LT. DOPEPS) THEN
      PHI(I) = PHIP +
     1 ((DOPE + DOPEPS) / (2.0D+00 * DOPEPS)) * (PHIN - PHIP)
      ELSE IF (DOPE.GT.0.D0) THEN
      PHI(I)=PHIN+DLOG(0.5D0*DOPE+DSQRT(1.D0+
     *(0.5D0*DOPE)**2))
      ELSE IF (DOPE .LT. 0.0D+00) THEN
      PHI(I)=PHIP-DLOG(-0.5D0*DOPE+DSQRT(1.D0+
     *(0.5D0*DOPE)**2))
      END IF
 1055 CONTINUE
C
