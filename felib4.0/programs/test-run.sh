...
default:
	foreach i ($PROGRAMS)
		...
		./$PROG <../data/$DATA >$RES1
        sed -e "/E[-+]/s/\(E\)\([-+]\)/D\2/g" <  $RES1 > $RES1.tmp
        sed -e "/E[-+]/s/\(E\)\([-+]\)/D\2/g" <  ../results/$RES2 > $RES2.tmp
		echo "Differencing results (old/new)"
		diff $RES2.tmp $RES1.tmp >\#diff$i
	...
	echo "*********************************************"
	if(-e $DIFFIL) then
        echo "*** Tests complete - Differences found"
	echo "*** Contents of difference file"
	cat $DIFFIL
	else
	echo "*** Tests complete - No differences found"
	endif
	echo "*********************************************"
endsw
